...
function foodMealTableInitialize (table) {
  ...
  if (meals !== null) {
    for (var i = 0; i < meals.length; i++) {
      addFoodToCorrectTable(meals[i].name, meals[i].calories, table)
    };
    $('tr').each(function(i) {
      $(this).removeClass('new-row');
      $( this ).addClass( "row" + i );
...
function addFoodToCorrectTable(name, calories, table) {
    switch(table) {
    case 'foods':
      break;
    case 'exercises':
      break;
    case 'breakfast':
      addFoodToMealDiaryTable(name, calories, table);
      break;
    case 'lunch':
      addFoodToMealDiaryTable(name, calories, table);
      break;
