...
function calculateCaloriesTotal(table) {
  ...
  var sum = 0;
  for(var i = 0; i < tableContents.children.length - 1; i++) {
...
function addToCheckBoxTable(name, calories, table) {
  var row = '<tr id="editable" class="new-row" ><td contenteditable="true">' +name+ '</td><td contenteditable="true">' +calories+ '</td><td><input id="'+name+'" type="checkbox"/><label for="'+name+'"></label></td></tr>'
  $('.'+table+' > tbody').prepend(row);
  makeEditable(table);
};
