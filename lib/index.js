...
function calculateRemainingCalories(table) {
  if (table === 'exercise') { return };

  var calories              = addCalories(table);
  var remainingCaloriesCell = $('#' + table + '-remaining-calories');
  switch(table){
    case 'breakfast':
      remainingCaloriesCell.text(400 - calories);
      break;
    case 'lunch':
      remainingCaloriesCell.text(600 - calories);
      break;
    case 'dinner':
      remainingCaloriesCell.text(800 - calories);
      break;
    case 'snacks':
      remainingCaloriesCell.text(200 - calories);
      break;
  }
}
...
function addToCheckBoxTable(name, calories, table) {
  var row = '<tr id="editable" class="'+table+name+'" ><td contenteditable="true">' +name+ '</td><td contenteditable="true">' +calories+ '</td><td><input type="checkbox" id="'+name+'"/><label for="'+name+'"></label></td></tr>'
  $('.'+table+' > tbody').prepend(row);
  var sharedFunctions  = new SharedFunctions();
  sharedFunctions.editable(table);
};
