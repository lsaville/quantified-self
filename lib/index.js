...
(function initializeDays() {
  ...
  if (!daysJSON) {
    ...
    //do massive table loading fest here
  }
})();
...
$('#day-back').on('click', function() {
  ...
  var dayBack = new Date(current - 1);
  displayDate(dayBack.toDateString());
  //Do crazy table loading here
})

$('#day-forward').on('click', function() {
  console.log('forward in time');
  var current    = new Date($('#date').text())
  var current = new Date(current.setDate(current.getDate() + 1))
  displayDate(current.toDateString());
  //Do crazy table loading here
})
